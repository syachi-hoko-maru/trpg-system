<template>
  <div class="balloon-div" :id="`${classRandId}`">
    <svg class="balloon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 114.88 218.08" v-if="balloon" @click="click">
      <g>
        <path class="parts"
          d="m57.44,142.47c-.35,0-.7-.01-1.05-.03v74.61c0,.56.46,1.03,1.03,1.03h.04c.56,0,1.02-.46,1.02-1.03v-74.61c-.34.02-.69.03-1.04.03Z" />
        <path class="color" :fill="fill"
        ...
        <rect class="parts" x="50" y="129.05" width="14.89" height="4.47" rx="2.23" ry="2.23" />
        <ellipse class="right" cx="35.62" cy="24.13" rx="16.67" ry="9.42"
...
const colors = ["#ed1c24", "#994fd1", "#65ba38", "#4f7ff0", "#ffa366", "#f57fc8", "#3bf5c6", "#fff419", "#bbf50c", "#8cfffb", "#d600b2", "#ff3700"]
...
const setUp = () => {
  ...
  setTimeout(() => {
    ...
    const animation = `wobbling_x ${rand < 2.5 || rand > 7.5 ? "0.7" : rand < 5.0 ? "0.8" : "0.9"}s ease-in-out 100 alternate, wobbling_y ${rand % 2 ? "1.1" : "1.2"}s ease-in-out 100 alternate, fly_high ${String(rand + 9)}s ease-in-out`
