<template>
  <v-card>
    ...
    <v-icon
      :icon="sound ? 'mdi-volume-high' : 'mdi-volume-off'"
      size="x-large"
      class="ma-2"
      @click="toggleSound"
    />
...
class Light {
  ...
  draw = (ctx: CanvasRenderingContext2D) => {
    ...
    const alpha =
      Math.floor(
        (this.life * 2 > this.maxlife ? 1 : (this.life * 2) / this.maxlife) * 10
      ) / 10;
    if (alpha < 0.1) {
      this.life = 0;
      return;
...
class Ball extends Particle {
  ...
  static launch = (burst: BurstFunction) => {
    ...
    particleArr.push(
      new Ball(
        Math.random() * (width - xPadding * 2) + xPadding,
        -4.5 + (Math.random() - 0.5),
        Math.random() * 500 + 500,
        burst
      )
...
const big: BurstFunction = (x: number, y: number) => {
  ...
  particleArr.push(
    new Particle(x, y, 0, 0, null, 20, (x, y) => {
      for (let i = 0; i < 30; i++) {
  ...
  for (let i = 0; i < countInGroup * 5; i++) {
    ...
    const rad =
      (Math.PI * 2 * ((i % countInGroup) + (group % 2) / 2)) / countInGroup;
    const v = 3;
    particleArr.push(
      new Particle(
        x,
...
const rest = (count: number): Program => ({
  fireworks: [],
  interval: count,
  count: 1,
});
const program: Program[] =
  mode === "normal"
    ? [
        ...
        {
          fireworks: [[twoColor], [twoColor], [twoColor], [three]],
          interval: 3,
          count: 6,
        },
        rest(5),
        ...
        {
          fireworks: [
            [twoColor, twoColor, twoColor],
            [twoColor, three],
            [three, three],
            [twoColorBig, twoColorBig],
            [twoColorBig, three],
          ],
          interval: 5,
          count: 6,
        ...
        rest(5),
        {
          fireworks: [[chiruno, chiruno, chiruno, chiruno]],
          interval: 3,
          count: 1,
        },
        rest(5),
        {
          fireworks: [
            [big, chiruno, chiruno, chiruno, chiruno],
            [four, chiruno, chiruno, chiruno, chiruno],
    ...
    : [
        {
          fireworks: [
            [twoColor, twoColor, twoColor],
            [twoColor, twoColor, twoColor],
            [twoColor, three, three],
            [twoColor, three, three],
            [twoColorBig, twoColorBig],
            [twoColorBig, three, three],
            [big, big],
            [big, four],
            [four, four],
          ],
...
program.forEach((p) => {
  ...
  while (i < p.count) {
    ...
    if (p.fireworks.length) {
      programArr.push(
        p.fireworks[Math.floor(Math.random() * p.fireworks.length)]
      );
    }
    // intervalだけnullを入れる
    let j = 0;
