...
const ghostStatus = ref(
  "noheld" as "exist" | "found" | "error" | "until" | "noheld"
);
const message = computed(() => {
  return ghostStatus.value === "noheld"
    ? "「ハロウィンデザイン」でないか切り替えた直後のため準備中です。ページを更新してください。"
    : ghostStatus.value === "until"
    ? "幽霊が隠れる準備中です。数秒で終わりますので、お待ちください。"
    : ghostStatus.value === "found"
    ? "このページの幽霊は見つけました。他のページも探してみてください！"
    : ghostStatus.value === "error"
    ? "エラーが発生しています。ページを更新してみてください。"
    : ghostStatus.value === "exist"
    ? "このページに幽霊が出現しています。探してみてください。"
    : "";
});
...
const setGhost = () => {
  ...
  try {
    const query =
      nowPage === "/"
        ? "section.card-div:not(.card-div .card-div)"
        : "div#center section.card-div:not(.card-div .card-div)";
    ...
    const px =
      window.scrollX +
      Math.floor(
        rect.left + (rect.right - rect.left - 16 * 2 - 40) * Math.random()
      );
