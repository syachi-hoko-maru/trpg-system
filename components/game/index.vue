<template>
  <div data-nosnippet>
    <card>
      <template #title> 2Dシューティングゲーム </template>
      <div v-if="!canvasError">
        ...
        <div v-if="mounted">
          <item-button @click="() => (showRule = !showRule)">{{
            showRule ? "遊び方を閉じる" : "遊び方を読む"
          }}</item-button>
          <andml :andmls="rule" v-if="showRule" />
          <item-button v-if="!gaming" @click="doCanvas"
            >ゲームを始める</item-button
          >
        </div>
      </div>
  ...
  moveX(direction?: number) {
    ...
    if (this.direction === 1) {
      if (this.x + this.dx < width - this.size) this.x += this.dx;
        ...
        this.x = Math.max(
          width * 2 - this.size * 2 - (this.x + this.dx),
          width - this.size
        );
        this.direction = -1;
        this.reflectAction();
...
class DropItem extends Entity {
  ...
  constructor() {
    ...
    if (!mine) {
      ...
      const x =
        Math.round(Math.random() * (width - this.size * 2 - mine.size * 10)) +
        this.size;
      this.x =
        x + this.size >= mine.x - mine.size * 5
          ? x + mine.size * 10 + this.size * 2
          : x;
      this.y = 10;
    }
  }
  ...
  auto() {
    ...
    if (mine) {
      if (
        (this.x - mine.x) ** 2 + (this.y - mine.y) ** 2 <=
        (this.size + mine.size) ** 2
      ) {
        this.hp = 0;
        this.effect();
      }
      ...
      for (let i = 0; i < this.shootingLevel; i++) {
        entityArray.push(
          new Bullet(
            this.x -
              this.size +
              ((this.size * (i + 1)) / (this.shootingLevel + 1)) * 2,
            y,
            this.team,
            dy
          )
...
class Enemy extends Machine {
  ...
  score(): number {
    return Math.floor(
      ((this.fullHp * this.dx) / this.ratio) * this.bulletDy * 100
    );
  }
  auto() {
    this.moveX();
  ...
  try {
    ...
    const addEnemy = () => {
      ...
      if (count >= t * 63 && count % t === 0) {
        entityArray.push(
          new Enemy(
            Math.round(count / t / 70) + 3,
            Math.round(count / t / 60) + 8,
            Math.round((30 * t * 60) / count),
            10
          )
        );
      }
    ...
    const draw = () => {
      ...
      entityArray.forEach((entity, i) => {
        ...
        if (
          entity.name === "enemy" &&
          entity.y > mine.y - entity.size - mine.size
        ) {
          gameover();
        }
        if (
          ...
          entity.y > (width * 2) / 3 - entity.size
        ) {
          if (entity.name === "mine") {
            if (mine.realHp <= 0) {
              gameover();
      ...
      addEnemy();
      if (
        count >= Math.round(1000 / interval) * 10 &&
        count % Math.round(1000 / interval) === 0
      ) {
        addItem();
      }
      score.value++;
