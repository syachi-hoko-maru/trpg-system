  ...
  <v-container class="pa-0">
    <v-row>
      ...
      <v-col cols="12" sm="7" class="py-0" @click="daychange">
        <item-form-select :form-setting="daySetting" :disabled="!monthSetting.value" />
      </v-col>
...
const monthSetting: Ref<FormSettingSelect> = ref({
  ...
  label: "年月",
  type: "select",
  selects: setNumberArray(12, today.getMonth() + 1).map(m => `${m}月`).concat(setNumberArray(today.getMonth() + 1, 1).map(m => `${today.getFullYear() + 1}年${m}月`)),
  value: ""
...
const getDays = () => setNumberArray(dayCount(monthSetting.value)).map(d => `${monthSetting.value.value}${d}日（${getYoubi(monthSetting.value, d)}）`)
