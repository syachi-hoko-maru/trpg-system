<template>
  <div v-if="pageSetting.page">
    <div style="height:58px">
      <v-pagination v-model="mpage" :length="pageSetting.page.length" rounded="circle" v-if="mounted" />
    </div>
    <v-alert class="my-5" type="info" :text="`「${pageSetting.title}」は長くなってしまったため、複数ページに分けております。`" />
    <card v-if="pageSetting.page.length && !top">
      <item-button v-if="page < pageSetting.page.length" :url="`${pageSetting.to}/${page + 1}`"
        normal-button>次のページへ</item-button>
      <item-button v-if="page > 1" :url="`${pageSetting.to}/${page - 1}`" normal-button>前のページへ</item-button>
      <item-button v-if="page >= pageSetting.page.length" :url="`${pageSetting.to}/1`" normal-button>最初から読む</item-button>
    </card>
  </div>
</template>
