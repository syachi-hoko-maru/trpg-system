<template>
  <div class="mb-3">
    ...
    <div class="my-2">
      <dl
        class="d-flex my-1"
        v-for="[key, value] of Object.entries(book).filter(
          ([key], i, arr) =>
            key !== 'title' &&
            key !== 'amazon' &&
            (key !== 'date' || arr.map((a) => a[0]).indexOf('dispDate') === -1)
        )"
      >
...
import type { Book, BookAttr } from "~/src/dict/new";
import { langList } from "~/src/dict/new";
