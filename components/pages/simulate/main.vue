      ...
      <v-card-actions class="pt-4">
        <v-form>
          <v-radio-group label="モード選択" v-model="mode" density="compact">
            <v-radio v-for="mo in modes" :label="modeSetting[mo].label" :value="mo" :key="mo" />
          </v-radio-group>
        </v-form>
      </v-card-actions>
      <card class="mt-0" :loading="!showSetting">
        ...
        <template v-if="showSetting">
          ...
          <v-window v-model="tab">
            <v-window-item v-for="t of modeSetting[mode].tabs" :value="t" :key="t">
...
onMounted(() => {
  setTimeout(() => {
    getQuerys()?.forEach(d => Object.entries(d).forEach(([key, value]) => {
      if (key === "mode") {
        if (typeof value === "string" && value !== mode.value && $mode.isMode(value)) {
          mode.value = value
        }
      }
      ...
      const setting = tabSetting[tabName].formSettings.value.find(f => f.name === match[2])
      if (!setting) return
