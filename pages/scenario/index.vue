<template>
  ...
  <card-list>
    <card v-for="sd of $scenarioData" :key="sd.id">
      <template #tbefore>
        <item-img :src="`${$templateText.basePath}/webp/${sd.img}`" :alt="`シナリオ「${sd.title}」のサムネイル画像`" />
      </template>
...
const andml1 = `
&1 このページについて
このページでは当ウェブサイトの管理人しゃちほこ丸が作ったTRPGシナリオを公開しております。
よければ見ていってください。
&br
なお、ページ下部には &em_おすすめシナリオの紹介 ページへのリンクもあるので、併せてご覧ください。
