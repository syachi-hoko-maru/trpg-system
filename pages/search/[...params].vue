    ...
    <template #default>
      ...
      <template v-if="!showPageSettings.length">
        ...
        <div class="my-2" v-if="sortSetting !== 'default'">
          ...
          <item-button @click="() => (sortSetting = 'default')"
            >デフォルト（関連度順）で表示</item-button
          >
        </div>
        <div v-else>他のキーワードで検索してください。</div>
      </template>
      <template v-else>
        <v-select
          label="並び順"
...
import { isSortType, sort, sortValues } from "~/src/search/sort";
import type { SortType } from "~/src/search/sort";
...
const showPageSettings = computed(() =>
  sortedResults.value.map(({ pageSetting }) => pageSetting)
);
...
const checkQuery = () => {
  ...
  if (rawTag || rawWord) {
    router.replace(
      "/search/" + [rawTag, rawWord].join("/") + `?sort=${rawSortSetting}`
    );
  }
  // そうでなければ検索方法を設定する
    ...
    meta: [
      {
        hid: "og:url",
        name: "og:url",
        content: $templateText.baseUrl + getNowPage(),
      },
      ...
      },
      {
        hid: "og:image",
        name: "og:image",
        content: $templateText.baseUrl + "/image/page-image/search.png",
      },
