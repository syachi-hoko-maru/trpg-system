<template>
  <Card>
    <template #title> あなたの運勢 </template>
    <Andml :andmls="andml1" />
    <ItemShare :text="shareText" :url="shareUrl" hashTag="ソドワおみくじ"
      >結果をツイート！</ItemShare
    >
...
import type { FortuneResult } from "~/src/simulater/omikuji/";
import { defaultVersion } from "~/src/simulater/omikuji/";
import { omikujiBasePath, sep } from "~/src/simulater/omikuji/";
...
const andml1 = ref([
  "システムエラーが発生しました。 &br &br もう一度実行してください。",
]);
const shareText = ref("");
const shareUrl = ref("");
...
　${fortuneResult.messageList[
    Math.floor(Math.random() * fortuneTypeListV1.length)
  ].slice(0, 20)}

詳しくはこちら！`;
  shareUrl.value = encodeURIComponent(
    `https://trpg.syachi.work${route.fullPath}`
  );
};
...
const mainLoop = async () => {
  await main();
  if (count < 10 && andml1.value.length < 20) {
    // if (route.query.id
