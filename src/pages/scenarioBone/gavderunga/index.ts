...
const npcBattle = `
&&data_NPCによる戦闘の補助（プレイヤー1人の場合）
生き残っているNPCは以下の支援をくれる。ラウンドの終了時、このうち1つを選んで使用しても良い。
ただし情報を3つとも調査済みのNPCからの支援はダメージ・回復量が2倍になる。
&br
${mainNpcList
  .map((n) => [
    `・${n.name}`,
    `　「${n.bonus[0] * 1.5}点ダメージ」もしくは「${
      n.bonus[1] * 1.5
    }点回復」のいずれか`,
  ])
  .flat()
...
${mainNpcList
  .map((n) => [
    `・${n.name}`,
    `　「${n.bonus[0]}点ダメージ」もしくは「${n.bonus[1]}点回復」のいずれか`,
  ])
  .flat()
  .join("\n")}
&&&
    ...
    event: [
      ...
      },
      {
        ...
        description: [
          ...
          "この際、すでに情報を3つ調査済みのNPCがいれば、彼は常にPCたちと一緒にいる。",
          "&br",
          "消化活動後に戦利品の決定を行っても良い。",
    ...
    event: [
      ...
      },
      {
        ...
        description: [
          ...
          "&br",
          ...mainNpcList.map((n) => `（${n.name}が犯人だった場合）「${n.dm}」`),
          "&br",
          "しかし、犯人が生きることを諦めたからだろうか。",
          "突然、犯人の体内の「ガブディランガの秘宝」は禍々しいオーラを放ち、回転を始めた。",
          "&br",
          "刹那、黒き雷鳴が轟く。",
          "魔動列車は火に包まれる。",
          "&br",
          "－戦闘開始－",
          "&br",
          "敵は1体のみ。前線エリアに配置。もし上級戦闘で遊ぶ場合には先頭のPCから10mの位置に配置すること。",
          "プレイヤー2人の場合、「部位：核」に剣のかけらが3つ入っている。",
